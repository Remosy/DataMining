...
class DocumentLevel:
    ...
    def train(self):
        X_train, X_test, y_train, y_test = train_test_split(self.train_data, self.labels, test_size=0.15,random_state=0)
        print("Train data has: "+str(len(X_train)))
        print("Test data has: "+str(len(X_test)))
        model = Sequential()
        model.add(Conv1D(64, 3, activation='relu', padding='same', input_shape=(45, 200),kernel_initializer='random_uniform'))
        ...
        model.add(Dense(40,kernel_regularizer=regularizers.l2(0.005),
                  activation='sigmoid',
